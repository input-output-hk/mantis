...
class BlockHeadersStorageSpec extends WordSpec with PropertyChecks with ObjectGenerators {
  ...
  "BlockHeadersStorage" should {
    ...
    "insert block header properly" in {
      forAll(Gen.listOfN(32, ObjectGenerators.blockHeaderGen)){ blockHeaders =>
        ...
        val totalStorage = blockHeaders.foldLeft(initialStorage){
          case (storage, blockHeader) => storage.put(blockHeader.hash, blockHeader)
        }
    ...
    "delete block header properly" in {
      forAll(Gen.listOfN(32, ObjectGenerators.blockHeaderGen)){ blockHeaders =>
        ...
        val storageAfterDelete = toDelete.foldLeft(totalStorage){
          case (storage, blockHeader) => storage.remove(blockHeader.hash)
        }
