...
class TransactionMappingStorageSuite extends FunSuite with ScalaCheckPropertyChecks with ObjectGenerators {
  test("TransactionMappingStorage insert") {
    forAll(Gen.listOf(byteStringOfLengthNGen(32))) { txByteArrayHashes =>
      ...
      txHashes.zip(txLocationList).foreach { case (txHash, txLocation) =>
        assert(txMappingStorage.get(txHash).contains(txLocation))
      }
    }
  }
  ...
  test("TransactionMappingStorage delete") {
    forAll(Gen.listOf(byteStringOfLengthNGen(32))) { txByteArrayHashes =>
      ...
      val txMappingStorageAfterDelete = toDelete.foldLeft(txMappingStorage) { case (recTxMappingStorage, (txHash, _)) =>
        recTxMappingStorage.remove(txHash)
      }

      toLeave.foreach { case (txHash, txLocation) =>
        assert(txMappingStorageAfterDelete.get(txHash).contains(txLocation))
      }
