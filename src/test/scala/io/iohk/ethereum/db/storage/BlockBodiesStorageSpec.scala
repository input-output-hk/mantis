  ...
  "BlockBodiesStorage" should {
    ...
    "insert block body properly" in {
      forAll(Gen.listOfN(32, ObjectGenerators.newBlockGen(secureRandom, chainId))) { newBlocks =>
        ...
        blocks.foreach { case NewBlock(_, block, _) =>
    ...
    "delete block body properly" in {
      forAll(Gen.listOfN(32, ObjectGenerators.newBlockGen(secureRandom, chainId))) { newBlocks =>
        ...
        val batchUpdates = toDelete.foldLeft(storage.emptyBatchUpdate) { case (updates, NewBlock(_, block, _)) =>
          updates.and(storage.remove(block.header.hash))
        }

        batchUpdates.commit()

        toLeave.foreach { case NewBlock(_, block, _) =>
          assert(storage.get(block.header.hash).contains(block.body))
        }
        toDelete.foreach { case NewBlock(_, block, _) => assert(storage.get(block.header.hash).isEmpty) }
      }
    }
    ...
    def insertBlockBodiesMapping(newBlocks: Seq[CommonMessages.NewBlock]): BlockBodiesStorage = {
      ...
      val batchUpdates = newBlocks.foldLeft(storage.emptyBatchUpdate) { case (updates, NewBlock(_, block, _)) =>
