...
class BlockBodyTest extends AnyWordSpec with Matchers {
  ...
  "BlockBodyTest" should {
    ...
    "hashCode and equals contract is held" in {
      ...
      val newBlockBody = Block3125369.body.withTransactions(oldBlockBody.transactionsAsIndexedSeq)

      oldBlockBody.hashCode shouldEqual newBlockBody.hashCode
    }

    "construct a reverseIterator" in {
      val nonces = Block3125369.body.transactionsAsIndexedSeq.map(_.tx.nonce)
      val reverseHashes = Block3125369.body.transactionReverseIterator.map(_.tx.nonce).toStream
      nonces.reverse shouldEqual reverseHashes
    }

    "enumerate the same way as zipWithIndex" in {
      val zipped = Block3125369.body.transactionsAsIndexedSeq.zipWithIndex
      val enumerated = Block3125369.body.transactionEnumerator.toStream
    ...
    "withTransactions should work the same way as copy" in {
      ...
      copy.transactionsAsIndexedSeq.size shouldEqual 0
      copy.numberOfUncles shouldEqual Block3125369.body.numberOfUncles

      val secondCopy = copy.withTransactions(Block3125369.body.transactionsAsSeq)
      Block3125369.body shouldEqual secondCopy
    }

    "findWhere should find first matched result" in {
      val headFound = Block3125369.body.findTransactionWhere(_ => true).map {  case (i, tx) => i -> tx.tx.nonce }
      headFound shouldEqual Some( 0 -> Block3125369.body.transactionIterator.next.tx.nonce )
    }

    "findWhere should return None when predicate is always false" in {
      val headFound = Block3125369.body.findTransactionWhere(_ => false).map {  case (i, tx) => i -> tx.tx.nonce }
    ...
    "getTransactionByIndex should return None on any index out of bounds" in {
      ...
      val seq = Block3125369.body.transactionsAsIndexedSeq
    ...
    "findWhere should return transaction and its correct index" in {
      val blockBody = Block3125369.body
      for (i <- 0 until 1000) {
        val correctIndex = Random.nextInt(blockBody.numberOfTxs)
        val correctTxn = blockBody.getTransactionByIndex(correctIndex).get
        val guess = blockBody.findTransactionWhere(_.tx.nonce == correctTxn.tx.nonce)
