...
class CheckpointBlockGeneratorSpec extends AnyFlatSpec with Matchers {
  ...
  it should "generate a proper block with checkpoint" in new TestSetup {
    ...
    val expectedDifficulty = difficultyCalculator
      .calculateDifficulty(parentBlock.number + 1, timestamp, parentBlock.header)
  ...
  trait TestSetup extends BlockchainConfigBuilder {
    ...
    val checkpointBlockGenerator = new CheckpointBlockGenerator(difficultyCalculator)
  }
}
