...
class FastSyncReceiptsRequestHandlerSpec extends FlatSpec with Matchers {
  ...
  "FastSyncReceiptsRequestHandler" should "handle successful response (and enqueue remaining receipts)" in new TestSetup {
    ...
    peerMessageBus.expectMsg(Subscribe(MessageReceivedClassifier(Set(Receipts.code), PeerSelector.WithId(peer.id))))
    ...
    peerMessageBus.expectMsg(Unsubscribe(MessageReceivedClassifier(Set(Receipts.code), PeerSelector.WithId(peer.id))))
  }
