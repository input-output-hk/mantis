...
class SyncStateSpec extends AnyFlatSpec with Matchers {
  ...
  "SyncState" should "prepend mpt nodes when enqueueing them" in {
    val syncState = SyncState(
      pivotBlock = Fixtures.Blocks.ValidBlock.header,
      pendingMptNodes = toStateMptNodeHash("1", "2", "3"),
      pendingNonMptNodes = toEvmCodeHash("a", "b", "c")
    )

    val resultingSyncState =
      syncState.addPendingNodes(toStateMptNodeHash("4", "5", "6")).addPendingNodes(toEvmCodeHash("d", "e", "f"))
