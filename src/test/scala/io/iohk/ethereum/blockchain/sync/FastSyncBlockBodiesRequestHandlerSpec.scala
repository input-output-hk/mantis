...
class FastSyncBlockBodiesRequestHandlerSpec extends FlatSpec with Matchers {
  ...
  "FastSyncBlockBodiesRequestHandler" should "handle successful response (and enqueue remaining hashes)" in new TestSetup {
    ...
    peerMessageBus.expectMsg(Subscribe(MessageReceivedClassifier(Set(BlockBodies.code), PeerSelector.WithId(peer.id))))
    ...
    peerMessageBus.expectMsg(Unsubscribe(MessageReceivedClassifier(Set(BlockBodies.code), PeerSelector.WithId(peer.id))))
  }
