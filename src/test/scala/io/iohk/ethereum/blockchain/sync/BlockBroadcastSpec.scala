...
import io.iohk.ethereum.network.p2p.messages.{CommonMessages, PV62, Versions}
...
class BlockBroadcastSpec
  ...
  it should "send a new block when it is not known by the peer (known by comparing chain weights)" in new TestSetup {
    ...
    //when
    blockBroadcast.broadcastBlock(BlockToBroadcast(newBlock), Map(peer -> initialPeerInfo))
  ...
  it should "send a new block when it is not known by the peer (known by comparing chain weights) (PV63)" in new TestSetup {
    ...
    val blockHeader: BlockHeader = baseBlockHeader.copy(number = initialPeerInfo.maxBlockNumber - 3)
    val newBlockNewHashes = NewBlockHashes(Seq(PV62.BlockHash(blockHeader.hash, blockHeader.number)))
    val peerInfo = initialPeerInfo.copy(remoteStatus = peerStatus.copy(protocolVersion = Versions.PV63))
    ...
    //when
    blockBroadcast.broadcastBlock(BlockToBroadcast(newBlock), Map(peer -> peerInfo))
  ...
  it should "send block hashes to all peers while the blocks only to sqrt of them" in new TestSetup {
    ...
    blockBroadcast.broadcastBlock(BlockToBroadcast(firstBlock), peersWithInfo)
  ...
  it should "not broadcast NewBlockHashes message when disable by configuration" in new TestSetup {
    ...
    val blockToBroadcast = BlockToBroadcast(newBlock)

    blockBroadcast.broadcastBlock(blockToBroadcast, Map(peer -> initialPeerInfo))
