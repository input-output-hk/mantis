...
import io.iohk.ethereum.network.p2p.messages.{Capability, CommonMessages, PV64, Versions}
...
class EtcHandshakerSpec extends AnyFlatSpec with Matchers {
  ...
  trait TestSetup extends SecureRandomBuilder with EphemBlockchainTestSetup {
    ...
    val initHandshakerWithoutResolver = EtcHandshaker(new MockEtcHandshakerConfiguration(Versions.PV64))
  ...
  trait LocalPeerSetup extends TestSetup {
    val localHello = Hello(
      ...
      capabilities = Seq(Eth64Capability, Eth63Capability),
