...
class EtcPeerManagerSpec extends AnyFlatSpec with Matchers {
  ...
  it should "update the peer total difficulty when receiving a PV64.NewBlock" in new TestSetup {
    ...
    //given
    val newBlock = PV64.NewBlock(baseBlock, initialPeerInfoPV64.chainWeight.increaseTotalDifficulty(1))
  ...
  trait TestSetup extends EphemBlockchainTestSetup {
    ...
    val initialPeerInfoPV64 = PeerInfo(
      remoteStatus = peerStatus.copy(protocolVersion = Versions.PV64),
      chainWeight = peerStatus.chainWeight,
      forkAccepted = false,
      maxBlockNumber = Fixtures.Blocks.Block3125369.header.number,
      bestBlockHash = peerStatus.bestHash
    ...
    def setupNewPeer(peer: Peer, peerProbe: TestProbe, peerInfo: PeerInfo): Unit = {
      ...
      peerEventBus.expectMsg(
        Subscribe(
          MessageClassifier(
            Set(BlockHeaders.code, NewBlock.code, NewBlockHashes.code),
