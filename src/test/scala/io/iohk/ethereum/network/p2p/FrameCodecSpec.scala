...
class FrameCodecSpec extends FlatSpec with Matchers {
  ...
  "FrameCodec" should "send and receive Ping message" in new SecureChannelSetup {
    ...
    val msg = Ping()
    val data = frameCodec.writeFrame(msg.code, ByteString(encode(msg)))
  ...
  it should "send message and receive a response" in new SecureChannelSetup {
    ...
    val ping = Ping()
    val pingData = frameCodec.writeFrame(ping.code, ByteString(encode(ping)))
    ...
    val pong = Pong()
    val pongData = remoteFrameCodec.writeFrame(pong.code, ByteString(encode(pong)))
