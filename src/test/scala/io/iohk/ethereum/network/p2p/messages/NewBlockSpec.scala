...
class NewBlockSpec extends AnyFunSuite with ScalaCheckPropertyChecks with ObjectGenerators with SecureRandomBuilder {
  ...
  test("NewBlock v63 messages are encoded and decoded properly") {
    forAll(newBlockGen(secureRandom, Some(chainId)).map(_.as63)) { newBlock =>
      val encoded: Array[Byte] = newBlock.toBytes
      val decoded: NewBlock = encoded.toNewBlock(NewBlock.code63)
      val newBlockWithoutCheckpointNumber =
        newBlock.copy(chainWeight = ChainWeight.totalDifficultyOnly(newBlock.chainWeight.totalDifficulty))
      assert(decoded == newBlockWithoutCheckpointNumber)
    }
  }

  test("NewBlock v64 messages are encoded and decoded properly") {
    forAll(newBlockGen(secureRandom, Some(chainId)).map(_.as64)) { newBlock =>
      val encoded: Array[Byte] = newBlock.toBytes
      val decoded: NewBlock = encoded.toNewBlock(NewBlock.code64)
  ...
  test("NewBlock messages are properly encoded") {
    val obtainEncoded = Hex.toHexString(newBlock.as63.toBytes)
    val expectedEncoded =
    ...
    val obtainEncoded2 = Hex.toHexString(newBlock2.as63.toBytes)
    val expectedEncoded2 =
      ""
  ...
  val newBlock = NewBlock(
    ...
    ChainWeight.totalDifficultyOnly(983040)
  )
