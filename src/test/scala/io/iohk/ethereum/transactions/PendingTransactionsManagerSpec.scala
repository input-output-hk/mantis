...
class PendingTransactionsManagerSpec extends FlatSpec with Matchers with ScalaFutures with DefaultPatience {
  ...
  trait TestSetup {
    ...
    val peer1 = new PeerImpl(peer1TestProbe.ref, peerMessageBus.ref)
    val peer2TestProbe = TestProbe()
    val peer2 = new PeerImpl(peer2TestProbe.ref, peerMessageBus.ref)
    val peer3TestProbe = TestProbe()
    val peer3 = new PeerImpl(peer3TestProbe.ref, peerMessageBus.ref)
