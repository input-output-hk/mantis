...
class StaticCallOpcodeSpec extends WordSpec with Matchers with PropertyChecks {
  ...
  "STATICCALL" should {
    ...
    "calling a program that executes a state-changing opcodes" should {
      val programsWithStateChangingOpcodes = Map(
        ...
        SELFDESTRUCT -> stateWithProgram(Assembly(
          PUSH20, fxt.extAddr.bytes,
          SELFDESTRUCT)
        ),
        SSTORE -> stateWithProgram(Assembly(
          PUSH1, 0,
          PUSH1, 10,
          SSTORE)
        )
      ...
      programsWithStateChangingOpcodes.foreach {
        case (op, worldState) =>
