...
class MemorySpec extends FunSuite with PropertyChecks with ObjectGenerators {
  ...
  test("store DataWord") {
    assert(Memory().store(0, fill(1)).buffer == fill(1).value)
    assert(Memory().store(2, fill(1)).buffer == ByteString(0, 0) ++ fill(1).value)
    assert(Memory().store(2, fill(1)).store(1, fill(2)).buffer == ByteString(0) ++ fill(2).value :+ 1)
  ...
  test("load DataWord") {
    ...
    assert(Memory(ByteString(0, 0) ++ fill(1).value).load(3).value == 0 +: ByteString(Array.fill[Byte](31)(1)))
  }
}
