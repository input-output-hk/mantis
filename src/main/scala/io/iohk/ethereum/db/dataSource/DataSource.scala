...
trait DataSource {
  ...
  /** This function obtains the associated value to a key, if there exists one. It assumes that caller
    * already properly serialized key. Useful when caller knows some pattern in data to avoid generic serialization.
    *
    * @param key the key retrieve the value
    * @return the value associated with the passed key
    */
