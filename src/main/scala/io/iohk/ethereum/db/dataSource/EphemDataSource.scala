...
class EphemDataSource(var storage: Map[ByteBuffer, Array[Byte]]) extends DataSource {
  ...
  private def updateOptimized(namespace: Namespace, toRemove: Seq[Array[Byte]], toUpsert: Seq[(Array[Byte], Array[Byte])]): Unit = synchronized {
  ...
  override def iterate(): Observable[(Array[Byte], Array[Byte])] = {
    Observable.fromIterable(storage.toList.map {case (key, value) => (key.array(), value)})
  }

  override def iterate(namespace: Namespace): Observable[(Array[Byte], Array[Byte])] = {
    val namespaceVals = storage collect {
      case (buffer, bytes) if buffer.array().startsWith(namespace) => (buffer.array(), bytes)
    }
