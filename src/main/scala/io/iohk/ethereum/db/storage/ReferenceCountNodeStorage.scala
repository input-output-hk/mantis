...
object ReferenceCountNodeStorage extends PruneSupport with Logger {
  ...
  /** Fetches snapshots stored in the DB for the given block number and deletes the stored nodes, referred to
    * by these snapshots, that meet criteria for deletion
    * (see [[ReferenceCountNodeStorage.getNodesToBeRemovedInPruning()]] for details).
    *
  ...
  /** Within snapshots stored for this block, it looks for Nodes that are not longer being used in order to remove them
    ...
    * by any later block (by examining [[StoredNode.lastUsedByBlock]] field).
    *
    * @param blockNumber  the number of the block for which nodes will be removed in pruning
    * @param snapshotKeys the list of node hashes which will be checked
    * @param nodeStorage  the storage of possible nodes
    * @return nodes that need to be remove in pruning
    */
  ...
  /** Wrapper of MptNode in order to store number of references it has.
    *
    * @param nodeEncoded      Encoded Mpt Node to be used in [[io.iohk.ethereum.mpt.MerklePatriciaTrie]]
    * @param references       Number of references the node has.
    *                         Each time it's updated references are increased and whenever it's deleted, decreased.
  ...
  /** Key to be used to store BlockNumber -> Snapshots Count
    *
    * @param blockNumber BlockNumber Tag
    * @return Key
    */
  ...
  /** Used to store a node snapshot in the db. This will be used to rollback a transaction.
    ...
    * @param storedNode stored node that can be rolled back.
    *                   If None, it means that node wasn't previously in the DB
    */
