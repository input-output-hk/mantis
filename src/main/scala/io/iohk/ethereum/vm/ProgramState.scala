...
case class ProgramState(
  ...
  def ownAccount: Account = world.getAccount(ownAddress).get  // FIXME: that nasty get again, but I'm starting to think it OK
  ...
  def withStorage(updated: Storage): ProgramState = {
    val world1 = world.saveStorage(ownAccount.storageRoot, updated)
    withWorld(world1)
  }
