...
class FilterManager(
  ...
  private def addFilterAndSendResponse(filter: Filter): Unit = {
    ...
    lastCheckBlocks += (filter.id -> blockchain.getBestBlock().get.number)
  ...
  private def getFilterLogs(id: BigInt): Unit = {
    ...
    filterOpt.foreach { _ =>
      lastCheckBlocks += (id -> blockchain.getBestBlock().get.number)
  ...
  private def getLogs(filter: LogFilter, startingBlockNumber: Option[BigInt] = None): Seq[TxLog] = {
    ...
    val bestBlockNumber = blockchain.getBestBlock().get.number
  ...
  private def getBlockHashesAfter(blockNumber: BigInt): Seq[ByteString] = {
    val bestBlock = blockchain.getBestBlock().get.number
