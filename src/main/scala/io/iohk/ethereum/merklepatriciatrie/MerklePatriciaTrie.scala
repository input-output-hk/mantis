...
class MerklePatriciaTrie[K, V](private val rootHash: Option[Array[Byte]],
      ...
      remove(root, keyNibbles) match {
        case NodeRemoveResult(true, Some(newRoot), nodesToRemoveFromStorage, nodesToUpdateInStorage) =>
          ...
          val afterDeleteDataSource = updateNodesInStorage(
            ...
            dataSource = dataSource,
            hashFn = hashFn)
          new MerklePatriciaTrie(Some(newRootHash), afterDeleteDataSource, hashFn)(kSerializer, vSerializer)
        case NodeRemoveResult(true, None, nodesToRemoveFromStorage, nodesToUpdateInStorage) =>
          val afterDeleteDataSource = updateNodesInStorage(
            ...
            dataSource = dataSource,
            hashFn = hashFn)
          new MerklePatriciaTrie(None, afterDeleteDataSource, hashFn)(kSerializer, vSerializer)
  ...
  private def get(node: Node, searchKey: Array[Byte]): Option[Array[Byte]] = node match {
    ...
    case extNode@ExtensionNode(sharedKey, next, _) =>
    ...
    case branch@BranchNode(children, terminator, _) =>
  ...
  private def put(node: Node, searchKey: Array[Byte], value: Array[Byte]): NodeInsertResult = {
    node match {
      ...
      case branchNode@BranchNode(children, optStoredValue, _) =>
  ...
  private def remove(node: Node, searchKey: Array[Byte]): NodeRemoveResult = (node, searchKey.isEmpty) match {
    ...
    case (BranchNode(children, None, _), true) => NodeRemoveResult(hasChanged = false, maybeNewChild = None)
    ...
    case (branchNode@BranchNode(children, optStoredValue, _), false) =>
      ...
      getChild(branchNode, searchKeyHead, dataSource) map { child =>
        ...
        NodeRemoveResult(hasChanged = false, maybeNewChild = None)
      }
    case (LeafNode(existingKey, storedValue, _), _) =>
    ...
    case (extensionNode@ExtensionNode(sharedKey, next, _), _) =>
  ...
  private def fix(node: Node, dataSource: DataSource, notStoredYet: Seq[Node], hashFn: HashFn): Node = node match {
    ...
    case extensionNode@ExtensionNode(sharedKey, next, _) =>
      val nextNode = extensionNode.next match {
        case Left(nextHash) =>
          ...
          notStoredYet.find(n => n.hash sameElements nextHash).getOrElse(
            getNextNode(extensionNode, dataSource) // We search for the node in the db
          )
      ...
      val newNode = nextNode match {
        ...
        // It's ok
        case BranchNode(subChildren, subOptValue, _) => node
      }
      newNode
    case _ => node
  }
