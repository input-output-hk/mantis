...
trait GetPeersHandler { this: BlockBroadcastActor =>
  ...
  /**
    * This function handles the [[io.iohk.ethereum.network.GetPeersHandler.RequestPeers]] message by asking the PeerManager for the current peers.
    */
  ...
  def handlePeersResponse(state: ProcessingState): Receive = {
    case PeersResponse(peers) if state.toBroadcastBlocks.nonEmpty =>
      self ! SendBlocksToPeers(peers.map(_.ref), state.toBroadcastBlocks)
