...
case class EtcHelloExchangeState(handshakerConfiguration: EtcHandshakerConfiguration)
  ...
  override def applyResponseMessage: PartialFunction[Message, HandshakerState[PeerInfo]] = { case hello: Hello =>
    ...
    if (handshakerConfiguration.protocolVersion == Versions.PV64 && hello.capabilities.contains(Eth64Capability))
      ...
      log.debug(
        s"Connected peer does not support eth ${Versions.PV63.toByte} / ${Versions.PV64.toByte} protocol. Disconnecting."
      )
  ...
  private def createHelloMsg(): Hello = {
    ...
    val capabilities =
      if (handshakerConfiguration.protocolVersion == Versions.PV64) Capabilities.All else Seq(Eth63Capability)
