...
/** Abstraction for a consensus protocol implementation.
  ...
  */
trait Consensus {
  /** The type of configuration [[io.iohk.ethereum.consensus.FullConsensusConfig.specific]]
  ...
  /** Returns the [[io.iohk.ethereum.consensus.blocks.BlockGenerator]] this consensus protocol uses. */
  def blockGenerator: BlockGenerator

  /** Starts the consensus protocol on the current `node`. */
  def startProtocol(node: Node): Unit

  /** Stops the consensus protocol on the current node. This is called internally when the node terminates. */
...
/** Internal API, used for testing.
  ...
  * but it is the developer's responsibility to maintain consistency
  * (though the particular consensus protocols we implement so far do their best in that direction).
  */
