...
import io.iohk.ethereum.sync.util.SyncUtils.{BlockchainState, randomAddress, retryUntilWithDelay}
...
trait FakePeer extends SecureRandomBuilder with TestSyncConfig {
  ...
  protected def getMptForBlock(block: Block): InMemoryWorldStateProxy = {
  ...
  def getCurrentState(): BlockchainState = {
    ...
    val currentTd = bl.getTotalDifficultyByHash(bestBlock.hash).getOrElse(throw new RuntimeException(s"block by hash: ${bestBlock.hash} doesn't exist"))
    BlockchainState(bestBlock, currentWorldState, currentTd)
  }

  protected def createChildBlock(parent: Block, parentTd: BigInt, parentWorld: InMemoryWorldStateProxy, plusDifficulty: BigInt = 0)(
