...
class ContractTest extends FlatSpec with Matchers {
  ...
  "Ledger" should "transfer ether" in {
    ...
    new LedgerImpl(VM, blockchainConfig, vmConfig).executeBlock(fixtures.blockByNumber(1), storage, validators) shouldBe noErrors
  }
  ...
  it should "deploy contract" in {
    ...
    //contract creation
    new LedgerImpl(VM, blockchainConfig, vmConfig).executeBlock(fixtures.blockByNumber(2), storage, validators) shouldBe noErrors
  }
  ...
  it should "execute contract call" in {
    ...
    new LedgerImpl(VM, blockchainConfig, vmConfig).executeBlock(fixtures.blockByNumber(3), storage, validators) shouldBe noErrors
  }
